...
export default function CreditPage() {
    ...
    const onReceiveRepayment = (values: z.infer<typeof receiveRepaymentSchema>) => {
        addCreditRepayment(values.amount, values.destination as 'cash' | 'bank');
    ...
    return (
        <AppLayout>
            <PageHeader
                ...
                description="Manage a running total of credit given and received."
            />
                    ...
                    <Card>
                        ...
                        <CardContent>
                             <p className="text-3xl font-bold font-headline text-primary">
                                {formatCurrency(currentOutstandingCredit)}
                            </p>
                        </CardContent>
                    </Card>
                    <Card>
                        ...
                        <CardContent>
                            <Tabs defaultValue="give">
                                <TabsList className="grid w-full grid-cols-2">
                                    <TabsTrigger value="give">Give Credit</TabsTrigger>
                                    <TabsTrigger value="receive">Receive Repayment</TabsTrigger>
                                </TabsList>
                                <TabsContent value="give" className="pt-4">
                                     <Form {...giveCreditForm}>
                                        <form onSubmit={giveCreditForm.handleSubmit(onGiveCredit)} className="space-y-4">
                                            <FormField control={giveCreditForm.control} name="amount" render={({ field }) => (
                                                <FormItem><FormLabel>Amount to Give</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>
                                            )} />
                                            <Button type="submit" className="w-full">Record Credit Given</Button>
                                        </form>
                                    </Form>
                                </TabsContent>
                                <TabsContent value="receive" className="pt-4">
                                     <Form {...receiveRepaymentForm}>
                                        <form onSubmit={receiveRepaymentForm.handleSubmit(onReceiveRepayment)} className="space-y-4">
                                             <FormField control={receiveRepaymentForm.control} name="amount" render={({ field }) => (
                                                <FormItem><FormLabel>Amount Received</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem>
                                            )} />
