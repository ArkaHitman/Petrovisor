...
const TransactionTable = ({ transactions }: { transactions: FundAnalysisOutput['categorizedTransactions']['identifiedPurchases'] }) => {
    ...
    const handleAdjust = () => {
        toast({
            title: "Action Queued",
            description: "These transactions will be processed and adjusted in your portfolio soon.",
        });
    };
                ...
                <TableBody>
                    {transactions.map((tx, index) => (
                        <TableRow key={index}>
                            ...
                            <TableCell>
                                <Badge variant={tx.fundSource === 'Manager Fund' ? 'destructive' : 'default'}>{tx.fundSource}</Badge>
                            </TableCell>
                    ...
                    <CardContent className="space-y-6">
                        {isAnalyzing ? (
                            ...
                            <div className="space-y-6">
                                ...
                                <Card>
                                    <CardHeader>
                                        <CardTitle className="text-lg">Detailed Transaction Breakdown</CardTitle>
                                        <CardDescription>Review transactions categorized by the AI.</CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                        <Accordion type="multiple" defaultValue={['purchases']} className="w-full">
                                            <AccordionItem value="purchases">
                                                <AccordionTrigger>
                                                    <div className="flex items-center gap-2">
                                                        <span>Identified Purchases</span>
                                                        <Badge variant="secondary">{analysisResult.categorizedTransactions.identifiedPurchases.length}</Badge>
                                                    </div>
                                                </AccordionTrigger>
                                                <AccordionContent>
                                                    <TransactionTable transactions={analysisResult.categorizedTransactions.identifiedPurchases} />
                                                </AccordionContent>
                                            </AccordionItem>
                                            <AccordionItem value="manager">
                                                <AccordionTrigger>
                                                    <div className="flex items-center gap-2">
                                                        <span>Manager Payments</span>
                                                        <Badge variant="secondary">{analysisResult.categorizedTransactions.managerPayments.length}</Badge>
                                                    </div>
                                                </AccordionTrigger>
                                                <AccordionContent>
                                            ...
                                            <AccordionItem value="credit">
                                                <AccordionTrigger>
                                                    <div className="flex items-center gap-2">
                                                        <span>Credit Payoffs</span>
                                                        <Badge variant="secondary">{analysisResult.categorizedTransactions.creditPayoffs.length}</Badge>
                                                    </div>
                                                </AccordionTrigger>
                                                <AccordionContent>
                                                    <TransactionTable transactions={analysisResult.categorizedTransactions.creditPayoffs} />
                                                </AccordionContent>
                                            </AccordionItem>
                                             <AccordionItem value="unidentified">
                                                <AccordionTrigger>
                                                    <div className="flex items-center gap-2">
                                                        <span>Unidentified Debits</span>
                                                        <Badge variant="destructive">{analysisResult.categorizedTransactions.unidentifiedDebits.length}</Badge>
                                                    </div>
                                                </AccordionTrigger>
                                                <AccordionContent>
                                                    <TransactionTable transactions={analysisResult.categorizedTransactions.unidentifiedDebits} />
                                                </AccordionContent>
                                            </AccordionItem>
                                        </Accordion>
                                    </CardContent>
                                 ...
                                 <Button variant="outline" onClick={handleReset}>
                                    <Trash2 className="mr-2 h-4 w-4" /> Reset & Start Over
                                </Button>
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center h-96 text-muted-foreground">
