...
**EXECUTION PLAN: "Source-of-Fund Tagging"**
...
**Step 2: Tag All Debits (This is CRITICAL)**
...
- For each debit transaction, subtract it from the remaining "Manager Fund". Tag this debit as "Manager Fund".
- ONCE the "Manager Fund" balance is fully depleted (reaches zero or less), ALL subsequent debits MUST be tagged as "Your Fund".
...
- For each month, calculate the values for the 'monthlyAnalysis' table.
  ...
  - \`profitEstimated\`: If cost of goods is not available, estimate profit as a reasonable percentage of sales revenue (e.g., 10-15%). State your assumption if you make one.
...
- The logic for debit tagging is the most important part. First exhaust the manager's fund, then use the owner's.
